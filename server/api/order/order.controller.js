...
var createOrder = function(shipping_info, customer_id, customer_group_id, email, customer_ip) {
	...
	var order_dict = {
		...
		'shipping_address_2': '',
		'shipping_country': shipping_info.country_d ? shipping_info.country_d.name : '中国',
		...
		'payment_address_2': '',
		'payment_country': shipping_info.country_d ? shipping_info.country_d.name : '中国',
...
var createOrderTotal = function(order_id = 0, shipping_info, cart) {
	...
	if(_.sum(cart.discount.promotions,'saved_amount') > 0) {
		_.forEach(cart.discount.promotions, (promotion) => {
			insert_coll.push(getOrderTotalDict(order_id, 'promotion', promotion.name+' - '+promotion.saved_amount, -promotion.saved_amount, 3));
		});
	}
...
export function create(req, res) {
	...
	shipping_info.total = cart.product_total_price + cart.shipment_fee - cart.discount.coupon.saved_amount - cart.discount.reward.saved_amount - cart.discount.voucher.saved_amount - _.sum(cart.discount.promotions, 'saved_amount');
